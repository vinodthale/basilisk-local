 ...
 * CORRECTED VERSION - Fixed for proper axisymmetric geometry
 *
 ...
 *
 * AXISYMMETRIC COORDINATE SYSTEM (axi.h):
 * - x = AXIAL coordinate (vertical/streamwise direction)
 * - y = RADIAL coordinate (perpendicular to axis, y ≥ 0)
 * - Axis of symmetry at y = 0
 * - Domain rotates around y = 0 axis
 *
...
// Use standard Basilisk includes
// If custom embed files exist (myembed.h, etc.), replace these lines
#include "two-phase.h"
#include "embed.h"
#include "vof.h"
#include "tension.h"
...
// Derived parameters - CORRECTED for axisymmetric coordinates
// In axi.h: x = axial (vertical), so plate position is in x-direction
#define PLATE_POS_X    (L0 - POOL_HEIGHT)           // Plate upper surface axial position
#define DROPLET_POS_X  (PLATE_POS_X + DROP_RELEASE) // Initial droplet center axial position
#define PLATE_POS_R    (L0 - PLATE_FROM_RB)         // Plate extent in radial direction (if needed)
...
// Note: Check your contact.h implementation - may need to be vector or scalar
scalar contact_angle[];
scalar edge_marker[];  // Marker for sharp edge region

int main() {
  size (L0);
  origin (0., 0.);  // Axis of symmetry at y = 0
  ...
  // CORRECTED: Gravity in -x direction (axial/vertical fall)
  // For droplet falling downward in axisymmetric: gravity in -x
  const face vector g[] = {-BO/WE, 0.};
  a = g;
  av = g;
  ...
  // Contact angle (if using standard contact.h, may need different assignment)
 ...
 * CORRECTED: In axi.h coordinates:
 * - Plate is perpendicular to x-axis (horizontal plate)
 * - Orifice is centered on axis (y = 0)
 * - Plate extends from r = 0 to r = R_max
 * - Orifice allows flow for r < R_ORIFICE
 */
event init (t = 0) {
  ...
  foreach_vertex() {
    ...
    double x_axial = x;   // Axial coordinate (vertical)
    double r_radial = y;  // Radial coordinate (horizontal distance from axis)

    // Plate upper and lower surfaces (in axial direction)
    double plate_top = PLATE_POS_X;
    double plate_bottom = PLATE_POS_X - PLATE_THICK;
    ...
    if (r_radial < r_orifice) {
      // Inside orifice region - fluid (allows flow through)
      phi_val = 1.0;
    } else {
      // Outside orifice - check if inside plate thickness
      if (x_axial <= plate_top && x_axial >= plate_bottom) {
        // Solid plate region
        phi_val = -(r_radial - r_orifice);
      } else {
  ...
  // CORRECTED: Initialize spherical droplet in axisymmetric coordinates
  // Sphere: r² + (x - x₀)² < R²
  // In axi.h: y is radial (r), x is axial
  fraction (f, sq(R_DROPLET) - sq(y) - sq(x - DROPLET_POS_X));
  boundary({f});

  // CORRECTED: Initialize edge marker for sharp edge regions
  foreach() {
    double x_axial = x;   // Axial coordinate
    double r_radial = y;  // Radial coordinate

    double plate_top = PLATE_POS_X;
    double plate_bottom = PLATE_POS_X - PLATE_THICK;

    // Mark cells near upper and lower sharp edges
    double dist_upper_edge = sqrt(sq(r_radial - R_ORIFICE) + sq(x_axial - plate_top));
    double dist_lower_edge = sqrt(sq(r_radial - R_ORIFICE) + sq(x_axial - plate_bottom));
  ...
  // Note: If contact_angle is vector, use contact_angle.x[] and contact_angle.y[]
  foreach() {
    if (edge_marker[] > 0.5) {
 ...
 * CORRECTED: Volume in axisymmetric must include 2π factor
 */
event logfile (i++) {
  double volume = 0.;
  ...
  foreach(reduction(+:volume)) {
    // Axisymmetric volume: dV = 2π * r * dx * dr
    // dv() already includes dx*dy, so multiply by 2π*y
    volume += f[] * 2.0 * pi * y * dv();
  }
 ...
 * CORRECTED: Track axial positions on the axis (y ≈ 0)
 * M: Leading interface (minimum x on axis)
 * N: Trailing interface (maximum x on axis)
 */
event interface_tracking (t += 0.01) {
  static FILE * fp = NULL;
  if (t == 0) {
    fp = fopen("interface_position_sharp_nondim.txt", "w");
    fprintf (fp, "# Time* (non-dim)\tTime*/T_g*\tLeading_X*(M)\tTrailing_X*(N)\n");
  }

  // CORRECTED: Find leading and trailing interface positions along axis (y ≈ 0)
  double x_min = L0, x_max = 0.;

  foreach() {
    if (y < 0.01 && f[] > 0.01 && f[] < 0.99) {  // Near axis (y≈0), at interface
      if (x < x_min) x_min = x;  // Leading edge (min axial position)
      if (x > x_max) x_max = x;  // Trailing edge (max axial position)
    }
  }

  fprintf (fp, "%g\t%g\t%g\t%g\n", t, t/T_GRAVITY_ND, x_min, x_max);
  fflush(fp);
}
...
event snapshots (t = {0.49*T_GRAVITY_ND, 0.73*T_GRAVITY_ND,
  ...
  // Also output the full field (in axisymmetric x-y coordinates)
